...
export default function Events() {
        ...
        <Container>
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400 text-sm sm:text-base md:text-lg">
              رویدادهای آینده و گذشته ما را مشاهده کنید
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-gray-800/30 rounded-2xl p-6 animate-pulse"
              >
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 bg-gradient-to-b from-[#0c0c22] to-black relative overflow-hidden">
      {/* Background Effects */}
            ...
            <Link href={`/events/${event.id}`} key={event.id} className="group">
              <div className="bg-gradient-to-br from-gray-800/40 to-gray-900/40 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/10 hover:border-white/20 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-2xl hover:shadow-purple-500/10 h-full flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h3 className="text-xl sm:text-2xl font-bold mb-3 text-white group-hover:text-blue-300 transition-colors line-clamp-2 text-right">
                    {event.title}
                  </h3>
